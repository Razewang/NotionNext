...
// Vercel 的 Serverless 运行环境中项目目录是只读的。Preview 环境的
// `BLOG.isProd` 为 false，原逻辑因此会误启用 FileCache 并产生 EROFS。
// 构建阶段仍然需要文件缓存；本地开发也保留原有的文件缓存行为。
const isVercelRuntime = Boolean(process.env.VERCEL || process.env.VERCEL_ENV)
const enableLocalCache =
  isBuildPhase || (!BLOG.isProd && !isVercelRuntime)
