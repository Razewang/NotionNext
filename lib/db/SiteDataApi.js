...
function applySerializableDefaults(db) {
  db.NOTION_CONFIG = db.NOTION_CONFIG || {}
  db.notice = db.notice ?? null
  db.siteInfo = db.siteInfo || getSiteInfo({})
  db.tagOptions = Array.isArray(db.tagOptions) ? db.tagOptions : []
  db.categoryOptions = Array.isArray(db.categoryOptions)
    ? db.categoryOptions
    : []
  db.customMenu = Array.isArray(db.customMenu) ? db.customMenu : []
  db.customNav = Array.isArray(db.customNav) ? db.customNav : []
  db.allPages = Array.isArray(db.allPages) ? db.allPages : []
  db.allMembers = Array.isArray(db.allMembers) ? db.allMembers : []
  db.allEvents = Array.isArray(db.allEvents) ? db.allEvents : []
  db.allNavPages = Array.isArray(db.allNavPages) ? db.allNavPages : []
  db.allLinkPages = Array.isArray(db.allLinkPages) ? db.allLinkPages : []
  db.latestPosts = Array.isArray(db.latestPosts) ? db.latestPosts : []
  db.postCount = Number.isFinite(db.postCount)
    ? db.postCount
    : db.allPages.length
}
